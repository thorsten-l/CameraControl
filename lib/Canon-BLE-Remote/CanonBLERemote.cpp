...
advdCallback::advdCallback(BLEUUID service_uuid, bool *ready_to_connect,
                           BLEAddress *address_to_connect)
{
...
void advdCallback::onResult(BLEAdvertisedDevice advertisedDevice)
{
  ...
  { // Check if device has service UUID available.
    ...
    if (service_uuid_wanted.equals(advertisedDevice.getServiceUUID()))
    {
      ...
      btCameraName = String(advertisedDevice.getName().c_str());
...
void CanonBLERemote::init()
{
  ...
  if (nvs.begin())
  {
    ...
    btCameraName = nvs.getString("cameraname");
...
void CanonBLERemote::scan(unsigned int scan_duration)
{
  ...
  advdCallback *advert_dev_callback =
      new advdCallback(SERVICE_UUID, &ready_to_connect, &camera_address);

  pBLEScan->setAdvertisedDeviceCallbacks(
      advert_dev_callback); // Retrieve a Scanner and set the callback we want
                            // to use to be informed when we have detected a new
                            // device.
  pBLEScan->setActiveScan(true);
  pBLEScan->start(scan_duration); // Specify that we want active scanning and
 ...
 *
 *  This function should be called only when you want to pair with the new
 * camera, and the camera is in remote paring screen. After paired, camera mac
 * address will be stored in ESP32 NVS for later connection use.
 *
...
bool CanonBLERemote::pair(unsigned int scan_duration)
{
  ...
  if (pclient->connect(camera_address))
  {
    ...
    if (pRemoteService != nullptr)
    {
      ...
      pRemoteCharacteristic_Pairing =
          pRemoteService->getCharacteristic(PAIRING_SERVICE);
      if ((pRemoteCharacteristic_Pairing != nullptr))
      {
        // Send request on pairing service from external device
        String device_name_ =
            " " + device_name + " ";          // Pairing message to send
        byte cmdPress[device_name_.length()]; // Stocking list of Bytes char of
                                              // the message
        device_name_.getBytes(cmdPress,
                              device_name_.length()); // message Parser
        cmdPress[0] = {0x03};
        pRemoteCharacteristic_Pairing->writeValue(
            cmdPress, sizeof(cmdPress),
            false); // Writing to Canon_pairing_service
        Serial.print("Camera paring success : ");
        Serial.println(btCameraName);
        delay(200);
        disconnect();
        BLEDevice::setEncryptionLevel(ESP_BLE_SEC_ENCRYPT_NO_MITM);
        delay(200);
        connect();
        nvs.setString("cameraname", btCameraName);
...
bool CanonBLERemote::connect()
{
  if (pclient->connect(camera_address))
  {
    ...
    if (pRemoteService != nullptr)
    {
      ...
      pRemoteCharacteristic_Trigger =
          pRemoteService->getCharacteristic(SHUTTER_CONTROL_SERVICE);
      if (pRemoteCharacteristic_Trigger != nullptr)
      {
        log_i("Camera connection Success");
        // disconnect();       // Disconnect remote from the camera every time
        // after action, as the real canon remote did.
        return true;
      }
      else
      {
...
bool CanonBLERemote::trigger()
{
  ...
  pRemoteCharacteristic_Trigger->writeValue(
      (uint8_t)(MODE_IMMEDIATE | BUTTON_SHUTTER_FULL_PRESSED),
      false); // Set the characteristic's value to be the array of bytes that is
              // actually a string.
  delay(200);
...
bool CanonBLERemote::focus()
{
  ...
  pRemoteCharacteristic_Trigger->writeValue(
      (uint8_t)(MODE_IMMEDIATE | BUTTON_SHUTTER_HALF_PRESSED),
      false); // Set the characteristic's value to be the array of bytes that is
              // actually a string.
  delay(200);
  pRemoteCharacteristic_Trigger->writeValue(MODE_IMMEDIATE, false);
