...
void CCUI::showHandlerMessageBox(
  CCUIMessageBoxType type, const char *message, bool closeButton,
  lv_event_cb_t eventCallback, const char *buttons[]
  ) 
{
  lv_obj_t *messageBox =
    lv_msgbox_create(NULL, messageBoxTypes[(int)type], message, buttons, closeButton);

  if ( eventCallback != nullptr )
  {
    lv_obj_add_event_cb(messageBox, eventCallback, LV_EVENT_VALUE_CHANGED, NULL);
  }
...
void CCUI::showHandlerMessageBoxNOYES(
  CCUIMessageBoxType type, const char *message, bool closeButton, 
...
void CCUI::showHandlerMessageBoxRETURN(
    ...
    lv_event_cb_t eventCallback, const char *buttons[])
{
  ...
  xTaskCreatePinnedToCore(&messageBoxTask, "messageBoxTask", 10000,
                          NULL, 1, NULL, 1);
}
